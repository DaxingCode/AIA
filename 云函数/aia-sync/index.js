...
async function handle(req) {
  ...
  if (action === 'push') {
    ...
    for (const r of list) {
      ...
      try {
        // 查询是否存在；若查询失败（文档不存在或权限），按不存在处理，继续 set
        let exist = null
        try {
          exist = await db.collection(COLLECTION).doc(r.id).get()
        } catch (getErr) {
          console.log('push get 失败，按不存在处理:', r.id, getErr)
        }
        console.log('push get 结果:', r.id, 'exist.data=', exist && exist.data)
        ...
        if (exist && exist.data) {
          ...
          if (base.updatedAt >= exist.data.updatedAt) {
            ...
            console.log('push 跳过旧记录:', r.id, '本地', base.updatedAt, '< 云端', exist.data.updatedAt)
          }
        } else {
  ...
  if (action === 'pull') {
    ...
    try {
      ...
      while (true) {
        ...
        // CloudBase 返回的文档主键是 _id，但 App 端按约定读 rec.id；统一映射避免 apply 时跳过全部记录。
        const normalized = data.map(r => ({ ...r, id: r.id || r._id }))
