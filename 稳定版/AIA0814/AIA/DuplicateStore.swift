...
struct DuplicatePayload {
    ...
    let source: RecognitionSource = .cloud
}
...
enum RecognitionPresent: Identifiable {
    ...
    case pending(RecognitionResult, rawText: String, image: UIImage?, source: RecognitionSource, presavedImageName: String?, screenshotShortcut: Bool)  // 新鲜识别，未入库
    var id: String {
        switch self {
        case .saved(let s): return "saved-\(s.id.uuidString)"
        case .duplicate(let d): return "dup-\(d.hash)"
        case .pending(let result, let rawText, _, _, _, _):
