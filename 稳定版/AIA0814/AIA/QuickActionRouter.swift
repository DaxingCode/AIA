...
enum QuickAction: String {
    ...
    case chat   = "com.aia.shortcut.chat"     // 问好记AI
    case todo   = "com.aia.shortcut.todo"     // 查待办
}

/// 桌面「快捷操作」widget 的 Link 触发 aia://voice|camera|chat|todo 短 host，
/// 归一为完整的 QuickAction（与长按 App 图标快捷操作同源）。
func quickActionFromWidgetRoute(_ route: String) -> QuickAction? {
    let prefix = "com.aia.shortcut."
    guard ["voice", "camera", "chat", "todo"].contains(route) else { return nil }
    return QuickAction(rawValue: prefix + route)
}
