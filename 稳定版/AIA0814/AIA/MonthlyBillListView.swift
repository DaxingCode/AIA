...
struct MonthlyBillListView: View {
    ...
    /// 识别来源标记（RecogSource 1:1 关联 Bill.syncId），用于每行显示「免费版AI识别/Pro版AI…」
    ...
    private func billRow(_ b: Bill) -> some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack(spacing: 5) {
                    Text("\(AppFormat.timeOnly.string(from: b.time)) · \(b.merchant)")
                        .lineLimit(1)
                    if let raw = recogSourceBySyncId[b.syncId],
