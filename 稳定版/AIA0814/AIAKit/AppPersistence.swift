...
public enum AppPersistence {
    ...
    /// 当前 schema：从 AIAMigrationPlan 的 v16 版本化 schema 构造（含 SleepSession），
    /// 必须与 `currentSchemaVersion`（16）保持一致，否则 SleepSession 等新增模型不在 container
    /// schema 里注册 → context.insert 静默丢弃、数据不落盘（删 App 重装后尤为明显）。
    public static var schema: Schema { Schema(versionedSchema: SchemaVersion16.self) }
