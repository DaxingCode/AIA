...
struct MyAccountView: View {
    ...
    @AppStorage(ICloudBackupManager.enabledKey) private var icloudEnabled = false
    ...
    private var profileHeader: some View {
        VStack(spacing: 12) {
            ZStack {
                ...
                // 默认展示好记AI头像（AIAvatar = app icon）。昵称首字不再作为头像，避免与首页好记AI形象割裂。
                Image("AIAvatar")
                    .resizable()
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Label("iCloud 自动备份", systemImage: "icloud.and.arrow.up")
                    ...
                    .foregroundStyle(.primary)
                Spacer()
                // 开关始终展示：iCloud 不可用时变灰禁用，但用户仍能看到「默认是关」
            ...
            if !icloudAvailable {
                HStack(spacing: 8) {
                    ...
                    Text("iCloud 云盘未开启，自动备份不可用。请进入「设置→Apple账户→iCloud→iCloud 云盘-打开」后重试。")
                        .font(AIATheme.Font.micro)
                        .foregroundStyle(AIATheme.muted)
                        .lineSpacing(2)
                }
    ...
    private func backupNow() {
        ...
        Task {
            ...
            if !ok {
                icloudBusy = false
                uploadProgress = nil
                showToast("iCloud备份失败。请进入「设置→Apple账户→iCloud→iCloud 云盘-打开」，且空间充足。若确认「已开启」且「空间充足」，依然备份失败，可尝试重启手机，或进入「手机设置→Apple账户→退出登录」然后再登录。")
                return
            }
