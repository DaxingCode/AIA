...
struct GlobalToastOverlay: View {
    ...
    var body: some View {
        ZStack {
            if let p = center.payload {
                if p.important {
                    // 居中大卡：图标 + 文字横向紧凑排列 + 强调描边 + 阴影 + 缩放弹出。
                    HStack(spacing: 10) {
                        if let icon = p.icon {
                            iconView(icon: icon, accent: p.accent)
                        }
                        Text(p.text)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 18, style: .continuous)
                            .stroke(p.accent ?? .white, lineWidth: 1.5)
                    )
    ...
    /// 图标识别：仅含字母/数字/点/横线的当成 SF Symbols 显示，否则当 emoji 文字。
    @ViewBuilder
    private func iconView(icon: String, accent: Color?) -> some View {
        let isSFSymbol = icon.allSatisfy {
            ($0.isLetter || $0.isNumber || $0 == "." || $0 == "-") && $0.isASCII
        }
        if isSFSymbol {
