...
// ---- 样例 E：任务给定的「真实 OCR 输出（用户反馈图，Vision 实测）」逐字端到端验证 ----
// 任务原文以 "..." 省略了栏间行；此处用同源真实 OCR（即样例 D 的完整文本）补全省略行，
// 以完整保留两栏错位：支付时间↔值 相隔约 11 行、收款方全称↔阿里云计算有限公司 相隔约 11 行、
// 顶部「阿里云」与「-10.00」分行。验证端到端 4 项条件，并确认本地命中（不 fallback 云端）。
let sampleE = """
21:42⑦
账单详情
支付时间
...
添加＞
"""
print("\n===== 样例 E：任务真实 OCR 端到端（验证不 fallback 云端 + 4 项条件）=====")
if let r = localParseBill(text: sampleE) {
    // 进入 if-let 即说明 localParseBill 返回非 nil → 本地已确定金额，不会回退云端视觉模型
    check("E.本地命中(不回退云端)", "hit", "hit")
    check("E.merchant", r.merchant, "阿里云")
    checkAmount("E.amount", r.amount, 10.0)
    checkContains("E.isoTime", r.isoTime, "2026-07-21T11:09:35")
    check("E.category", r.category, "云服务")
    print("   raw:", r)
} else {
    failCount += 1
    print("❌ FAIL  E: localParseBill returned nil → 会 fallback 云端（不符合宽松兜底设计）")
}

// ---- 样例 F：宽松兜底验证 — 只有金额、无商户标签、无时间线索，也应返回非 nil（商户回退"账单"、时间回退当前） ----
let sampleF = """
账单详情
-10.00
"""
print("\n===== 样例 F：商户/时间缺失的宽松兜底（验证不再 fallback 云端）=====")
if let r = localParseBill(text: sampleF) {
    check("F.本地命中(不回退云端)", "hit", "hit")
    check("F.merchant(缺失→账单)", r.merchant, "账单")
    checkAmount("F.amount", r.amount, 10.0)
    print("   raw:", r, "  ← 商户缺失回退\"账单\"、时间缺失回退当前时间，符合设计")
} else {
    failCount += 1
    print("❌ FAIL  F: localParseBill returned nil → 会 fallback 云端（不符合宽松兜底设计）")
}
