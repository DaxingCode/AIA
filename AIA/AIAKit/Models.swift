...
// MARK: - 每日健康指标快照（正式数据库，替代旧 ManualHealthStore 的 UserDefaults 方案）
// 把每天手动录入 / HealthKit 自动拉取的健康指标（步数/睡眠/运动/活动热量/静息心率）
// 存进 SwiftData（App Group 共享 store），Widget 等独立进程可直接读同一份 SQLite，
// 不再依赖手动进程 buildSnapshot 中转、也不受 UserDefaults 跨进程读取限制。
// 每个 (dayTs, source) 一条记录；source = "manual" | "hk"，与旧 .hk 后缀槽位语义一致。
// 云同步仍走 type="manualHealth"（只同步手动 source，hk 永不进云），由 CloudSyncManager 读取本表组装。
