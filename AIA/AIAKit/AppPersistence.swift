...
public enum AppPersistence {
    ...
    /// 必须与 `schema` 实际引用的 VersionedSchema（SchemaVersion19）保持一致，
    ...
    /// 当前 schema：从 AIAMigrationPlan 的 v20 版本化 schema 构造（含 DailyHealthMetric.weight/height/bmi 等），
    /// 必须与 `currentSchemaVersion`（20）保持一致，否则新增模型不在 container
    /// schema 里注册 → context.insert 静默丢弃、数据不落盘（删 App 重装后尤为明显）。
    public static var schema: Schema { Schema(versionedSchema: SchemaVersion20.self) }
