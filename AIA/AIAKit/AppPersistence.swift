...
public enum AppPersistence {
    ...
    /// 必须与 `schema` 实际引用的 VersionedSchema（SchemaVersion21）保持一致，
    /// 否则日志/排查时版本号错乱，掩盖真实的迁移失败。
    // >>> CHANGE-[2026-09-16 09:44:24]-[补登记RecogSource到schema] 开始
    // v20 → v21：补登记漏掉的 RecogSource（详见 AIAMigrationPlan.SchemaVersion21）。
    ...
    /// 当前 schema：从 AIAMigrationPlan 的 v21 版本化 schema 构造（v20 + RecogSource），
    /// 必须与 `currentSchemaVersion`（21）保持一致，否则新增模型不在 container
    /// schema 里注册 → context.insert 静默丢弃、数据不落盘（删 App 重装后尤为明显）。
    // >>> CHANGE-[2026-09-16 09:44:24]-[补登记RecogSource到schema] 开始
    // v21：RecogSource 补登记（修复 iOS 27 HistoryObserver 冷启崩溃）。
    public static var schema: Schema { Schema(versionedSchema: SchemaVersion21.self) }
    // <<< CHANGE-[2026-09-16 09:44:24]-[补登记RecogSource到schema] 结束
