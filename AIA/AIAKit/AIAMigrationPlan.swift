...
// 原因：ChatMessage 新增可选字段 actionRouteRaw（默认 nil，lightweight 迁移安全）。
//       纯新增可选字段，v17→v18 走 lightweight 自动填充默认值 nil。
...
enum AIAMigrationPlan: SchemaMigrationPlan {
    ...
    // >>> CHANGE-[2026-08-20 15:30:00]-[小记查询跳转按钮] 开始
    // 原因：ChatMessage 新增可选字段 actionRouteRaw（默认 nil）。v17→v18 纯新增可选字段，lightweight 安全。
    // 回退：删除本 stage + 撤销 schemas/stages 中的 v18 引用。
