...
struct VoiceRecordView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .alert("提示", isPresented: Binding(get: { localError != nil }, set: { if !$0 { localError = nil } })) {
    ...
    private func finishAndRecognize() {
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    let session = RecognitionSaver.autoSave(result: output.result, rawText: output.rawText, image: nil, context: context)
                    isProcessing = false
