...
struct HealthGoalsView: View {
    ...
    private func recordBodyData() {
        ...
        do {
            let h1 = HealthMetric()
            h1.metric = "身高"; h1.value = String(format: "%.1f", heightCm); h1.unit = "cm"; h1.date = now
            context.insert(h1)
            let h2 = HealthMetric()
            h2.metric = "体重"; h2.value = String(format: "%.1f", weightKg); h2.unit = "kg"; h2.date = now
            context.insert(h2)
            let m = heightCm / 100
        ...
        VStack(spacing: 0) {
            ScrollView {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    FitnessGoalCard(selection: $fitnessGoalRaw)
                        .onChange(of: fitnessGoalRaw) { _, _ in markProfileChanged() }
...
// MARK: - 健身目标：减脂 / 增肌 / 维持（供饮食分析页做热量/蛋白目标对比）

private struct FitnessGoalCard: View {
    @Binding var selection: String   // "cut" / "bulk" / "maintain"

    private var binding: Binding<FitnessGoal> {
        Binding(
            get: { FitnessGoal(rawValue: selection) ?? .maintain },
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    Text(NSLocalizedString("fitness.goal.title", comment: ""))
                        .font(AIATheme.Font.footnote.weight(.medium))
                    Text(NSLocalizedString("fitness.goal.subtitle", comment: ""))
                        .font(AIATheme.Font.micro)
            ...
            SegmentedPicker(
                options: FitnessGoal.allCases.map { (value: $0, label: $0.label) },
                selection: binding
            )
        }
        .padding(12)
