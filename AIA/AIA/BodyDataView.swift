...
struct BodyDataView: View {
    ...
    private func save(height: Double, weight: Double) {
        let now = Date()
        context.insert(HealthMetric(metric: "身高", value: String(format: "%.1f", height), unit: "cm", date: now))
        context.insert(HealthMetric(metric: "体重", value: String(format: "%.1f", weight), unit: "kg", date: now))
        let m = height / 100
