...
enum RecognitionSaver {
    ...
    static func dueDate(from dueString: String?) -> Date {
        ...
        if dueString.range(of: #"[T\s]\d{1,2}:\d{2}"#, options: .regularExpression) != nil,
           let d = AppFormat.isoLocal.date(from: dueString) ?? AppFormat.iso.date(from: dueString) {
            return d
        }
    ...
    ///    image（截图/相册/分享/快捷指令）：当天 0:00（避免截图拍摄时间污染真实交易时间）。
    static func billTime(from time: String?, merchant: String?, category: String?, entryOrigin: String) -> Date {
                ...
                if baseCal > 0 || basePro > 0 || baseCar > 0 || baseFat > 0
                    ...
                    let foodDate: Date = {
                        if let d = f.date {
                            ...
                            if d.count > 10, let parsed = AppFormat.isoLocal.date(from: d) ?? AppFormat.iso.date(from: d) {
                                return parsed
                            }
...
/// 全部命中类别都无有效处理、或全部未识别 → 不插入消息（.nothing）。
    ...
    /// 文字永远只有一条、且 createdAt 早于所有卡片，保证「好好记先发文字 → 再发卡片；
    /// 一次识别多卡时文字仍只一条」。
    /// - Parameters:
...
static func insertPendingBubble(result: RecognitionResult, context: ModelContext,
    ...
    /// 注意：HealthKit 写入需主线程，但**绝不能用 `await MainActor.run` 阻塞当前后台流程**——
    /// 否则当 App 在前台时，后台 context.save() 触发的 NSManagedObjectContextDidSaveNotification 会让
    /// 主线程 @Query 忙于合并，腾不出手执行 MainActor.run 闭包，造成「后台等主线程、主线程等后台」互锁死结，
    /// 表象即 Siri 一句话后 App 卡死 + 转圈不停。
    /// 改为 fire-and-forget（Task { @MainActor in ... } 不 await），后台写流程立即走完，HealthKit 同步晚几百毫秒无所谓。
    private static func performSilentSideEffects(session: SavedSession, savedTypes: Set<String>) {
        if savedTypes.contains("food"), session.foodCaloriesTotal > 0 {
            Task { @MainActor in HealthManager.shared.saveCaloriesConsumed(session.foodCaloriesTotal, date: .now) }
        }
