...
// 无感截屏识别的核心：用 App Intents 暴露一个「小记自动记账、记待办、记饮食」系统动作。
// 用户在「快捷指令」里建「截屏」个人自动化，动作选「运行 小记自动记账、记待办、记饮食」，
...
struct ProcessScreenshotIntent: AppIntent {
    static var title: LocalizedStringResource = "小记自动记账、记待办、记饮食"
...
// 让「小记自动记账、记待办、记饮食」动作随 App 安装即自动出现在「快捷指令」App 里，
