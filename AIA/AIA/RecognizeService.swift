...
struct RecognizeService {
    ...
    static func queryFood(name: String) async throws -> FoodRef? {
        ...
        // 云端模型/权威表可能把状态注释带在名字里，如「四喜丸子（熟，约值）」「米饭（熟）」。
        // 显示名只需要简洁食物名，统一去掉全角/半角括号注释后再入库/展示。
        let cleanedName = RecognizeService.cleanCloudFoodName(foodName)
        return FoodRef(name: cleanedName,
                       kcal: calories,
    ...
    /// 清洗云端返回的食物名：去掉「（熟，约值）」「(raw)」等括号状态注释，保留简洁显示名。
    private static func cleanCloudFoodName(_ name: String) -> String {
        var s = name
        // 全角括号及其中内容
    ...
    private static func hasNutritionTable(in lines: [String]) -> Bool {
        ...
        // 项目关键词：含常见 OCR 形近/错字白名单（"能里/能重" 等），避免整表被误判为非营养表走云端。
        let itemSignals = ["能量", "热量", "蛋白质", "脂肪", "碳水化合物",
                           "能里", "能重", "能号", "能重", "蛋臼质", "蛋日质", "脂防", "碳木化合物"]
        ...
        // 「每100」放宽：OCR 常把 "每1o0g"/"每100 9" 读错，用模糊匹配容错。
        let hasHeader = lines.contains { line in
            let l = line.localizedCaseInsensitiveContains("每") &&
                line.range(of: #"每\s*\d*\s*[o0]\s*0?\s*[g克]"#, options: .regularExpression) != nil
            let alt = line.localizedCaseInsensitiveContains("每100") ||
                      line.localizedCaseInsensitiveContains("每 100") ||
                      line.localizedCaseInsensitiveContains("per 100")
            return l || alt
        }
    ...
    private static func localParseNutritionTable(observations: [VNRecognizedTextObservation]) -> RecognitionResult? {
        ...
        let labelKeywords: Set<String> = ["能量", "热量", "蛋白质", "脂肪", "碳水化合物", "碳水化台物", "碳水化食物", "糖类", "碳水", "钠", "钙", "项目", "每100"]
    ...
    // MARK: - 图像质量评估（拍照/选图不清晰友好提示）

    /// 照片质量问题分类。`.none` 表示清晰（或不足以影响识别）。
    /// 用于识别「没结果/失败」时，在对话页给出「图糊了/太暗了」这类针对性提示，
    /// 而非笼统的「识别失败」，让用户知道是拍摄问题而非 App 坏了。
    enum ImageQualityIssue: String {
        case blurry   // 模糊：对焦没对准 / 手抖
        case dark     // 太暗：光线不足
        case glare    // 过曝 / 反光太强
        case tooSmall // 分辨率过低 / 图太小
        case none     // 清晰（或质量问题不显著）
    }

    /// 评估单张图片的拍摄质量。计算成本低（纯 CPU、在调用方后台线程跑即可），
    /// 不改变识别结果；只作为「识别没结果时」的辅助提示依据。
    ///
    /// 算法：
    /// - 模糊度：灰度图做 3×3 拉普拉斯卷积后求方差（Variance of Laplacian）。
    ///   方差越小越糊（对焦失败 / 手抖），是业界判断「糊」最经典最快的方法。
    /// - 亮度/过曝：取缩小图的像素平均亮度（0~1）。过低=太暗，过高=过曝反光。
    /// - 分辨率：短边像素 < 阈值视为太小。
    ///
    /// 阈值留有余量：宁可少提示、别误伤正常图（误判糊图会惹烦用户）。
    static func assessImageQuality(image: UIImage) -> ImageQualityIssue {
        guard let cg = image.cgImage else { return .none }
        let w = cg.width, h = cg.height
        // 分辨率过低：短边 < 480px（一张糊的小图识别必然不准）。
        let shortSide = min(w, h)
        if shortSide < 480 { return .tooSmall }

        // 缩到最长边 ~256 做质量评估，足够判断糊/暗且更快。
        let targetLong = 256
        let scale = CGFloat(targetLong) / CGFloat(max(w, h))
        let tw = max(1, Int(CGFloat(w) * scale))
        let th = max(1, Int(CGFloat(h) * scale))
        // 直接在灰度 context 上把原图缩绘出来（CG 绘制时自动转灰度），保留明度信息用于亮暗/反光判断。
        ...
        guard let grayImg = ctx.makeImage() else { return .none }

        // 亮度统计 + 拉普拉斯方差
        guard let data = grayImg.dataProvider?.data,
              let ptr = CFDataGetBytePtr(data) else { return .none }
        let bytesPerRow = grayImg.bytesPerRow
        let count = tw * th
        var sum: UInt64 = 0
        var minV: UInt8 = 255, maxV: UInt8 = 0
        // 拉普拉斯卷积 3x3 核（8 邻域减中心 8 倍），在灰度缩图上逐像素算二阶差分方差。
        var lapSum: Int64 = 0
        var lapSqSum: Int64 = 0
        var lapCount: Int = 0
        for y in 0..<th {
            for x in 0..<tw {
                ...
                if v < minV { minV = v }
                if v > maxV { maxV = v }
                // 拉普拉斯：仅在内部像素计算（边界跳过，不影响统计显著性）
                if x > 0, x < tw - 1, y > 0, y < th - 1 {
                    let up = Int(ptr[(y - 1) * bytesPerRow + x])
                    let down = Int(ptr[(y + 1) * bytesPerRow + x])
                    let left = Int(ptr[y * bytesPerRow + x - 1])
                    let right = Int(ptr[y * bytesPerRow + x + 1])
                    let center = Int(v)
                    let lap = up + down + left + right - 4 * center
                    lapSum += Int64(lap)
                    lapSqSum += Int64(lap) * Int64(lap)
                    lapCount += 1
                }
            }
        }
        let mean = Double(sum) / Double(count) / 255.0          // 平均亮度 0~1
        let lapVar: Double = lapCount > 0
            ? (Double(lapSqSum) / Double(lapCount)) - pow(Double(lapSum) / Double(lapCount), 2)
            : 0

        // 阈值（经 XS Max/模拟器余量考量，偏宽松，避免误伤）：
        // - 拉普拉斯方差 < 18 → 明显模糊
        // - 平均亮度 < 0.10 → 太暗；> 0.92 且对比极低（maxV-minV 小）→ 过曝/反光
        if lapVar < 18 { return .blurry }
        if mean < 0.10 { return .dark }
        if mean > 0.92, (maxV - minV) < 40 { return .glare }
        return .none
    }
